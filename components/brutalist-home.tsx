...
export function BrutalistHome() {
  ...
  useEffect(() => {
    ...
    const dateString = new Date(now.getTime() - now.getTimezoneOffset() * 60_000)
      .toISOString()
      .slice(0, 10)
    // eslint-disable-next-line react-hooks/set-state-in-effect -- one-time read of the client's local date; this route is statically prerendered, so the date must come from the client, not build time
