  ...
  return (
    <PageShell eyebrow={eyebrow} title={title} description={description}>
      <section className="py-20 sm:py-28"><div className="mx-auto grid max-w-7xl items-center gap-12 px-4 sm:px-6 lg:grid-cols-2 lg:px-8">
        ...
        <div><span className="text-sm font-semibold uppercase tracking-widest text-accent">Who it is for</span><h2 className="mt-3 font-heading text-3xl font-bold uppercase sm:text-4xl">Training that meets you where you are</h2><div className="brand-rule mt-5 h-1 w-24 rounded-full" /><p className="mt-5 text-lg leading-relaxed text-muted-foreground">{idealFor}</p><ul className="mt-8 grid gap-4 sm:grid-cols-2">{benefits.map((benefit) => <li key={benefit} className="brand-panel flex gap-3 rounded-xl border p-4"><CheckCircle2 className="mt-0.5 size-5 shrink-0 text-accent" /><span>{benefit}</span></li>)}</ul><Button size="lg" className="mt-8 font-heading font-semibold uppercase" render={<a href="/contact" />}>Book a free trial</Button></div>
      </div></section>
    </PageShell>
  )
}
